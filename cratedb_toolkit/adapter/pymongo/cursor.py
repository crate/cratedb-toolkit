    ...
    class AmendedCursor(Cursor[_DocumentType]):
        ...
        def __send_message(self, operation: Union[_Query, _GetMore]) -> None:
            ...
            if self.__ordering is not None:
                order_by = []
                for column, order in self.__ordering.items():
                    if order == 1:
                        order_str = "ASC"
                    elif order == -1:
                        order_str = "DESC"
                    else:
                        raise ValueError(f"Unknown value for sorting order: {order}")
                    order_by.append(f"{column} {order_str}")
                sql += f" ORDER BY {', '.join(order_by)}"
            if isinstance(operation, _Query):
                if operation.ntoskip not in [0, -1]:
                    sql += f" OFFSET {operation.ntoskip}"
                if operation.limit not in [0, -1]:
