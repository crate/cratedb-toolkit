...
def write_sql_statements(settings) -> str:
    """
    Generates SQL statements for runtime configurable settings.
...
class OutputFormatter(FlexibleFormatter):
    ...
    def to_markdown(self):
        """
        Generates a Markdown table representation of the settings.
    ...
    def to_sql(self):
        """
        Converts extracted settings to SQL statements.
...
class SettingsExtractor(GenericProcessor):
    ...
    def acquire(self):
        # Extract settings
        """
        Acquires settings from CrateDB documentation and generates SQL statements.
        
        Extracts settings using extract_cratedb_settings() and assigns them to the
        thing attribute. If no settings are found, logs an error and exits the program.
        Otherwise, generates SQL statements for runtime configurable settings and
        returns the SettingsExtractor instance.
        """
    ...
    def render(self, format_: Union[str, OutputFormat]):
        """
        Renders settings using the specified format and logs runtime configurable counts.
        
        Calls the superclass's render method with the given format, computes the number of runtime configurable settings, logs this count, and returns the instance to allow method chaining.
