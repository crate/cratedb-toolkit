-- $ cat ibis/ci/schema/postgres.sql @ cc40bf5
...
\COPY diamonds FROM 'diamonds.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
\COPY astronauts FROM 'astronauts.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
\COPY batting FROM 'batting.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
\COPY awards_players FROM 'awards_players.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
\COPY functional_alltypes FROM 'functional_alltypes.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
\COPY geo FROM 'geo.csv' WITH (FORMAT CSV, HEADER TRUE, DELIMITER ',');
...
-- HSTORE does not work with AWS DMS.
-- DROP TABLE IF EXISTS map_hstore CASCADE;
-- CREATE TABLE map_hstore (kv HSTORE);
-- INSERT INTO map_hstore VALUES
--     ('a=>1,b=>2,c=>3'),
--     ('d=>4,e=>5,c=>6');
